...
## Directory Structure
~/.persistent-memory/ ├── _index.md # Index of all on-demand files (auto-maintained) ├── _core/ # Always loaded — essential context for every conversation │ ├── profile.md # Identity, background, goals │ └── preferences.md # Collaboration style, communication preferences, work habits ├── projects/ # On-demand: specific project details └── notes/ # On-demand: knowledge, frameworks, learnings
...
Read all files in _core/.
Read _index.md.
Briefly acknowledge what context is loaded.
Load On-Demand
Trigger: conversation topic matches an entry in _index.md.

Identify which file(s) are relevant based on _index.md summaries.
Check file size:
...
No need to announce — use the information naturally.
Save / Update
...
Determine: _core/ or on-demand?
Read the target file first.
Draft the content and show the user: what will be saved, which file, how it will be worded.
Wait for user to confirm or adjust.
Save. If on-demand file: update _index.md summary.
If the command is vague (e.g., "update memory" / "更新记忆"):

Scan the conversation and identify candidates — look for both:
...
Present a numbered list: what you propose to save, where, and why.
Wait for user to confirm, modify, or add items.
Save only what the user approves. Update _index.md if needed.
...
Create the directory structure (_core/, projects/, notes/, _index.md).
Save the content to the appropriate file.
Briefly explain: "I've set up a memory system. I'll remember this across conversations. Say 'load memory' at the start of future conversations to activate it."
No formal setup wizard. Memory builds naturally through conversation.

Memory Status
Trigger: "memory status" / "记忆状态" / "what's saved" / "存了什么".

Read _index.md and list all _core/ files.
For each file: name + one-line summary.
Show total count and approximate size.
Core Rules
Always read before writing. Never modify a file you haven't read in this conversation.
User decides what to save. Suggest, never auto-save silently.
Confirm every change. After any edit, state what changed and where.
Don't fabricate. If unsure about memory contents, read the file. Don't guess.
Keep files clean. Update existing entries rather than appending duplicates. Remove outdated info when updating.
Keep core small. Only put universally needed context in _core/. Topic-specific details go to on-demand files.
Index always in sync. Every save/update/delete to on-demand files must update _index.md.
No sensitive data. If user tries to store passwords or API keys, warn them that memory files are plain text.
Maintain summaries for large files. When a file reaches 100 lines or more, add or update a ## Summary section (5–10 lines) at the top of the file. This summary should capture the key points so the AI can decide whether to read the full content.
